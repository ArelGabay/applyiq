...
function readStoredAnalysis(source: string | null) {
  if (source !== "api" || typeof window === "undefined") {
    return null;
  }
...
export function AnalysisResultView() {
  ...
  const analysis = source === "api" ? storedAnalysis ?? defaultAnalysis : localAnalysis;
  ...
  const sourceLabel = isApiResult ? "API mock result" : "Local mock result";
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600">
            Mock analysis for {resume}.{" "}
            {isApiResult
              ? "Results came from the local FastAPI mock endpoint."
